...
import styles from './ProjSearchBar.less';

const { RangePicker } = DatePicker;

// 搜索组件
class ProjSearchBar extends Component {
  constructor(props) {
    super(props);
    this.state = ({
...
render() {
  ...
  return (
    <div className={styles.main}>
      ...
      />
      <Input placeholder='关键字：项目名称、项目说明' onKeyPress={this.handleEnterKey} value={likeKeyWord} onChange={this.handleKeyWordChange}/>
      <Cascader
        allowClear={false}
        options={typeList}
        value={[businessCode]}
      ...
      <Button className={styles.btn} type='dashed' icon='plus' onClick={this.createNewProj}>新建项目</Button>
    </div>
  );
}
}

export default ProjSearchBar;
